...
class BAFTAAgent(MovieAgent):
    """Agent that provides BAFTA Best Film winners."""
    ...
    async def collect(self, context: AgentContext) -> SourcePayload:
        ...
        if not rows:
            return SourcePayload(metadata={"notes": "BAFTA dataset missing"})
        ...
        for row in rows:
            ...
            movies.append(
                MovieCandidate(
                    movie_id=f"bafta:{title.lower().replace(' ', '_')}",
                    title=title,
                    year=year,
                    source_tags=["bafta", "british", "award"],
                    evidence=[f"BAFTA Best Film ({year})"],
                )
            )

        return SourcePayload(
            movies=movies,
            metadata={"notes": f"Loaded {len(movies)} BAFTA winners"},
        )

    def _load_data(self) -> list[dict]:
        if self._memory_store:
            try:
                cached_data, _ = self._memory_store.get_catalog_cache("bafta")
                if cached_data:
                    return cached_data
            except Exception:
                pass

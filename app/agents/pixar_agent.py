...
class PixarAgent(MovieAgent):
    """Agent that provides Pixar Animation Studios films."""
    ...
    async def collect(self, context: AgentContext) -> SourcePayload:
        ...
        if not rows:
            return SourcePayload(metadata={"notes": "Pixar dataset missing"})
        ...
        for row in rows:
            ...
            movies.append(
                MovieCandidate(
                    movie_id=f"pixar:{title.lower().replace(' ', '_')}",
                    title=title,
                    year=year,
                    source_tags=["pixar", "animation", "family"],
                    evidence=["Pixar Animation Studios"],
                )
            )

        return SourcePayload(
            movies=movies,
            metadata={"notes": f"Loaded {len(movies)} Pixar films"},
        )

    def _load_data(self) -> list[dict]:
        if self._memory_store:
            try:
                cached_data, _ = self._memory_store.get_catalog_cache("pixar")
                if cached_data:
                    return cached_data
            except Exception:
                pass

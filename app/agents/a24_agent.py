...
class A24Agent(MovieAgent):
    """Agent that provides A24 studio films."""
    ...
    async def collect(self, context: AgentContext) -> SourcePayload:
        ...
        if not rows:
            return SourcePayload(metadata={"notes": "A24 dataset missing"})
        ...
        for row in rows:
            ...
            movies.append(
                MovieCandidate(
                    movie_id=f"a24:{title.lower().replace(' ', '_')}",
                    title=title,
                    year=year,
                    source_tags=["a24", "indie"],
                    evidence=["A24 Studio Film"],
                )
            )

        return SourcePayload(
            movies=movies,
            metadata={"notes": f"Loaded {len(movies)} A24 films"},
        )

    def _load_data(self) -> list[dict]:
        if self._memory_store:
            try:
                cached_data, _ = self._memory_store.get_catalog_cache("a24")
                if cached_data:
                    return cached_data
            except Exception:
                pass

...
class FilmNoirAgent(MovieAgent):
    """Agent that provides classic film noir."""
    ...
    async def collect(self, context: AgentContext) -> SourcePayload:
        ...
        if not rows:
            return SourcePayload(metadata={"notes": "Film Noir dataset missing"})
        ...
        for row in rows:
            ...
            movies.append(
                MovieCandidate(
                    movie_id=f"noir:{title.lower().replace(' ', '_')}",
                    title=title,
                    year=year,
                    source_tags=["film_noir", "classic", "crime"],
                    evidence=["Classic Film Noir"],
                )
            )

        return SourcePayload(
            movies=movies,
            metadata={"notes": f"Loaded {len(movies)} film noir classics"},
        )

    def _load_data(self) -> list[dict]:
        if self._memory_store:
            try:
                cached_data, _ = self._memory_store.get_catalog_cache("film_noir")
                if cached_data:
                    return cached_data
            except Exception:
                pass

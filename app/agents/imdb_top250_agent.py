...
class IMDbTop250Agent(MovieAgent):
    """Agent that provides IMDb Top 250 movies."""
    ...
    async def collect(self, context: AgentContext) -> SourcePayload:
        ...
        if not rows:
            return SourcePayload(metadata={"notes": "IMDb Top 250 dataset missing"})
        ...
        for i, row in enumerate(rows, 1):
            ...
            movies.append(
                MovieCandidate(
                    movie_id=f"imdb250:{title.lower().replace(' ', '_')}",
                    title=title,
                    year=year,
                    source_tags=["imdb_top250", "classic"],
                    evidence=[f"IMDb Top 250 #{i}"],
                )
            )

        return SourcePayload(
            movies=movies,
            metadata={"notes": f"Loaded {len(movies)} IMDb Top 250 films"},
        )

    def _load_data(self) -> list[dict]:
        if self._memory_store:
            try:
                cached_data, _ = self._memory_store.get_catalog_cache("imdb_top250")
                if cached_data:
                    return cached_data
            except Exception:
                pass
